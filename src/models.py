...
class Transfer(pydantic.BaseModel):
    ...
    from_id: int = Field(..., description="Transfer from this account's id")
    to_id: int = Field(..., description="Transfer to this account's id")
    amount: float = Field(..., description="Transfer's amount. Always positive")
...
class Balances(pydantic.BaseModel):
    ...
    credits: float = Field(
        0, description="Sum of all credits made to this account, since its creation"
    )
    debits: float = Field(
        0, description="Sum of all debits made to this account, since its creation"
    )
    balance: float = Field(0, description="Difference between credits and debits")
